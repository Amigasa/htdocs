...
export default {
  ...
  setup() {
    ...
    onMounted(load);
    async function createUser() { const name = prompt('Введите имя'); const email = prompt('Email'); const role = prompt('Роль (admin, operator, client)', 'client'); const password = prompt('Пароль (мин 6)'); if (!name || !email || !password) { alert('Заполните поля'); return; } const res = await api.createUser({ username: email.split('@')[0], name, email, role, password }).catch(err => { alert('Ошибка: ' + err.message); return null; }); if (res && res.success) { alert('Создан'); load(); } }
    async function editUser(id) { const res = await api.getUsers().catch(() => ({ users: [] })); const u = res.users.find(x => x.id === id); if (!u) return; const name = prompt('Новое имя', u.name); const email = prompt('Новый email', u.email); const role = prompt('Роль', u.role); const updated = await api.updateUser(id, { name, email, role }).catch(err => { alert('Ошибка: ' + err.message); return null; }); if (updated && updated.success) { alert('Обновлено'); load(); } }
